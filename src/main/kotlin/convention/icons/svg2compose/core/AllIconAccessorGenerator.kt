...
import convention.icons.svg2compose.GeneratedGroup
...
public class AllIconAccessorGenerator(
  ...
  private fun createPropertySpecForNamed(
    ...
    val allIconsParametersFromGroups =
      childGroups.map { "%M.$allAssetsNamedPropertyName.mapKeys { \"\${%M.groupName}.\${it.key}\"}" }
    ...
    val allIconProperty = PropertySpec.builder(allAssetsNamedPropertyName, allIconsType)
      ...
      .getter(FunSpec.getterBuilder().withBackingProperty(allIconsBackingProperty) {
        addStatement(
          ...
          *(childGroups.flatMap {
            listOf(
              groupAllIconsMember(it),
              groupAllIconsMember(it)
            )
          } + iconProperties).toTypedArray()
        )
      }.build())
      .build()
