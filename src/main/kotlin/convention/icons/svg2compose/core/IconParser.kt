...
import convention.icons.svg2compose.core.vector.FillType
import convention.icons.svg2compose.core.vector.PathParser
import convention.icons.svg2compose.core.vector.StrokeCap
import convention.icons.svg2compose.core.vector.StrokeJoin
...
public class IconParser(private val icon: Icon) {

  public fun parse(): Vector {
          ...
          when (parser.name) {
            PATH -> {
              ...
              val strokeCap = parser.getAttributeValue(null, STROKE_LINE_CAP)
                ?.let { StrokeCap.entries.find { strokeCap -> strokeCap.name.equals(it, ignoreCase = true) } }
              ...
              val strokeJoin = parser.getAttributeValue(null, STROKE_LINE_JOIN)
                ?.let { StrokeJoin.entries.find { strokeJoin -> strokeJoin.name.equals(it, ignoreCase = true) } }
            ...
            GRADIENT -> {
              ...
              val lastPath =
                currentGroup?.paths?.removeAt(currentGroup.paths.lastIndex) ?: nodes.removeAt(nodes.lastIndex)
